-- =====================================================
-- admin_adjust_user_balance — update description + type labels
-- ADD  → description "Deposit",  type 'bonus'
-- SUBTRACT → description "SUBTRACT", type 'penalty'
-- Everything else identical to migration 029.
-- =====================================================
...
BEGIN
  ...
  -- 8. Determine transaction type and description based on action
  IF v_normal_action = 'ADD' THEN
    v_tx_type := 'bonus';
    v_tx_desc := 'Deposit — ' || v_normal_currency || ' — ' || v_clean_reason;
  ELSE
    v_tx_type := 'penalty';
    v_tx_desc := 'SUBTRACT — ' || v_normal_currency || ' — ' || v_clean_reason;
  END IF;

  -- 9. Create ledger transaction (unique reference for idempotency)
  ...
  INSERT INTO transactions (
    ...
    p_user_id, v_tx_type, v_signed_amount, v_ledger_currency, NULL,
    'admin_adjustment', auth.uid(), v_ref,
    v_tx_desc,
    'completed'
  ) RETURNING id INTO v_tx_id;

  -- 10. Store audit trail with full metadata
