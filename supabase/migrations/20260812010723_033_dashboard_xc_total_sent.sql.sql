-- Add xc_total_sent to get_dashboard (sum of XC sent via user-to-user transfers)
...
BEGIN
  ...
  SELECT COALESCE(sum(-amount), 0) INTO v_xc_total_sent FROM transactions
  WHERE user_id = auth.uid() AND type = 'xc_transfer_sent' AND currency = 'XC';
