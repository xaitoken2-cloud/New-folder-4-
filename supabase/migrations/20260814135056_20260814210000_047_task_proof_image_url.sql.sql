/*
# Add proof_image_url to tasks table

1. Schema Changes
   - Add nullable `proof_image_url text` column to `tasks` table.
   - This stores an optional reference image URL that advertisers can attach
     to show users what proof they should submit.

2. Function Changes
   - Drop and recreate `ad_create_task_campaign` to accept a new
     `p_proof_image_url text DEFAULT NULL` parameter and persist it
     into the new column during campaign creation.

3. Security
   - No RLS policy changes. The column is writable only through the
     existing SECURITY DEFINER function (advertiser-scoped).
*/

-- 1. Add nullable column
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS proof_image_url text;

-- 2. Drop and recreate ad_create_task_campaign with proof_image_url param
DROP FUNCTION IF EXISTS ad_create_task_campaign(
  text, text, text, text, text, numeric, text, boolean, text, integer, integer, numeric, text[]
);
